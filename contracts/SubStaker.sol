 ...
 *      can be transferred. Splitting Lista's voting power therefore needs a second account that
 *      stakes in its own name, which is all this is. It holds the validators marked `subValidator`
 *      on the manager; the manager holds the rest.
 *
 ...
 *
 *      The staking entry points mirror `IStakeHub`'s signatures, so the manager has one call site
 *      that differs only by target address.
 */
